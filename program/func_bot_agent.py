...
class BotAgent:
    ...
    def check_order_status_by_id(self,order_id):
        ...
        # If order cancelled move onto next Pair
        if order_status == "CANCELED":
            print(f"{self.market_1} vs {self.market_2} - Order canceled...")
            self.order_dict["pair_status"] = "FAILED"
            return "failed"

        # If order not filled wait until order expiration
        ...
        if order_status_m2 != "live":
            ...
            try:
                close_order = place_market_order(
                    self.client,
                    market=self.market_1,
                    side=self.quote_side, # quote so that it's on the opposite side
                    size=self.base_size,
                ...
                if order_status_close_order != "FILLED":
                    ...
                    # !! SEND MESSAGE IF EXECUTION FAILS
                    send_message(f"Failed to execute. Error code: 100")
                ...
                # !! SEND MESSAGE IF EXECUTION FAILS
                send_message(f"Failed to execute. Error code: 101")
