...
def open_positions(client): 
    ...
    """
        ...
        Store trades for managing trades on exit function
    """

    # Load cointegrated pairs
    df = pd.read_csv("cointegrated_pairs.csv")
    
    # Get markets from referincing of min order size, tick size etc
    ...
    for index, row in df.iterrows():
        ...
        # Get ZScore
        if len(series_1) > 0 and len(series_1) == len(series_2): # check
            spread = series_1 - (hedge_ratio * series_2) # you could fetch it again from the coint function
            ...
            if abs(z_score) >= ZSCORE_TRESH:
                ...
                # Place trade
                if not is_base_open and not is_quote_open:  # !!! Might edit to only NOT place the same exact trade
                    
                    # Determine side
                    base_side = "BUY" if z_score < 0 else "SELL"
                    quote_side = "BUY" if z_score > 0 else "SELL"

                    # !!! CAN MAKE IT BETTER?  this one below !!!
                    # Get acceptable price in string format with correct number of decimals
                    base_price = series_1[-1] # -1 is the latest price - could make another API call
                    quote_price=series_2[-1]
                    accept_base_price = float(base_price) * 1.01 if z_score < 0 else float(base_price) * 0.99
                    accept_quote_price = float(quote_price) * 1.01 if z_score > 0 else float(base_price) * 0.99
                    ...
                    # !!! YOU CAN ALSO CHECK FOR MAX ORDER SIZE !!!
                    ...
                    if check_base and check_quote:
                        ...
                        if free_collateral < USD_MIN_COLLATERAL:
                            print("ERROR --> free collateral is below the minimum treshold")
                            break
                        ...
                        if bot_open_dict["pair_status"] == "LIVE":
                            ...
                            del(bot_open_dict)   # we don't need it anymore, so delete it to free up some memory
